...
フロー内のノード集合は、`connections` によって定義される有向非循環グラフ（DAG: Directed Acyclic Graph）として順序集合となります。各ノード間の接続（`connections`）が順序関係を規定し、循環（サイクル）は明示的に排除されます。このため、任意のノード列はトポロジカル順序で一意に定まり、履歴や分岐の管理が可能です。

- connections: ノード間の順序関係を定義するペア（from, to）の集合
- ノード集合: connectionsによる順序集合（DAG）として解釈
